...
## Load the data
Data_Sero_RiskScore6 = pandas.read_csv('SeroScore_FcR_Lasso_wRisk6/SeroScore.csv',index_col=0)
...
#Temp_index = np.where(Data_Age['x'] == "male")[0]

K_folder = 5
################################################## Create the folder
root = "SeroScore_ROC_Lasso_Risk6_v2"
...
for feature_index in [342044-1]: #range(Data_Sero_RiskScore6.shape[1]):
    ...
    for iteration in range(50):  
        ...
        ###### 4. ROC plot
        viz = plot_roc_curve_woPred_drawfigure(X_train, y_train, name='Iteration {}'.format(iteration), alpha=0.3, lw=0.4, ax=ax)
    ...
    tprs_lower, tprs_upper = st.norm.interval(confidence=0.95, loc=np.mean(tprs, axis=0), scale=st.sem(tprs, axis=0)) 
    ...
    plt.savefig(os.path.join(directory, sel_features_saved + '.Sero.Data.LassoFeatures.HyperParameters_MoreSample_V4.pdf'), format="pdf", bbox_inches="tight")
    plt.show()