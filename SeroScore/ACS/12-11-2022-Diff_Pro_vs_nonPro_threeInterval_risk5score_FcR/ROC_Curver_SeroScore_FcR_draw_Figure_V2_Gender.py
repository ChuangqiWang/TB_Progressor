...
Data_Age = pandas.read_csv('Progressor_allmonths/Sex.csv', index_col = 0)
Temp_index = np.where(Data_Age['x'] == "female")[0]

K_folder = 5
################################################## Create the folder
root = "SeroScore_ROC_Lasso_FeMale"
...
for feature_index in [290872-1]: #range(Data_Sero_RiskScore6.shape[1]):
    ...
    for iteration in range(50):  
        ...
        train_index = pandas.read_csv('Classification_RF_LASSO/idx_train_' + str(iteration+1) + '.csv')
        ...
        train_index = intersection(train_index, Temp_index)
        test_index = intersection(test_index, Temp_index)
        ...
        np.savez(os.path.join(directory, 'Iteration_' + str(iteration) + '_pred_actual_test_new.npz'), X_test, yhat, y_test)
        
        
        ###### 4. ROC plot
        viz = plot_roc_curve_woPred_drawfigure(yhat, y_train, name='Iteration {}'.format(iteration), alpha=0.3, lw=0.4, ax=ax)
        #viz = plot_roc_curve_woPred_drawfigure(X_test, y_test, name='Iteration {}'.format(iteration), alpha=0.3, lw=0.4, ax=ax)
        interp_tpr = np.interp(mean_fpr, viz.fpr, viz.tpr)
        interp_tpr[0] = 0.0
        if(~np.isnan(viz.roc_auc)):     #Ignore the nan values
            tprs.append(interp_tpr)
            aucs.append(viz.roc_auc)
        
            
    np.savez(os.path.join(directory, 'AUCparameter' + '_new.npz'), tprs, aucs, viz)
    ...
    ax.fill_between(mean_fpr, tprs_lower, tprs_upper, color='lightgray', alpha=.7,
                    label=r'$\pm$ 1 std. dev.')
    
    #Confidence Interval estimated by t distribution
    #create 95% confidence interval for population mean weight
    #low_CI, upper_CI = st.norm.interval(confidence=0.95, loc=np.mean(tprs, axis=0), scale=st.sem(tprs, axis=0)) 
    #ax.fill_between(mean_fpr, low_CI, upper_CI, color='g', alpha=.7,
    ...
    plt.savefig(os.path.join(directory, sel_features_saved + '.Sero.Data.LassoFeatures.HyperParameters_Moresamples_V3.pdf'), format="pdf", bbox_inches="tight")
