...
Data_Group = pandas.read_csv('../../7_Experiments_11-05-2021/12-11-2022-Diff_Pro_vs_nonPro_threeInterval_risk5score_FcR/Progressor_Allmonths/Data_group.csv',index_col=0)
Data_Age = pandas.read_csv('../../7_Experiments_11-05-2021/12-11-2022-Diff_Pro_vs_nonPro_threeInterval_risk5score_FcR/Progressor_Allmonths/Sex.csv', index_col = 0)
Temp_index = np.where(Data_Age['x'] == "male")[0]

K_folder = 5
################################################## Create the folder
root = "SeroScore_ROC_Lasso_Male"
...
for feature_index in [0]: #range(Data_Sero_RiskScore6.shape[1]):
    ...
    for iteration in range(50):  
        ...
        # train_index = pandas.read_csv('../../8_Experiments_01-12-2022_GC6/Diff_Pro_vs_nonPro_threeInterval_risk5score_FcR/Classification_RF_LASSO/idx_train_' + str(iteration+1) + '.csv')
        ...
        # X_test = X[test_index, ]
        # y_test = y[test_index]
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=iteration, stratify = y)
        yhat = X_train
