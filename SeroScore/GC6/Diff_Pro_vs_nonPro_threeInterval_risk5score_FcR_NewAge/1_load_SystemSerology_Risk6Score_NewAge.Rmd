...
```
1. load the new Age information
```{r}
...
Data_metainfo_New <- cbind(Data_metainfo, Age.meta[Matched.index, c(4, 7, 8)])
```
...
library(ggplot2)
library(ggridges)
ggplot(Data_metainfo_New, aes(x=age, fill=Data_group)) +
...
ggplot(Data_metainfo_New, aes(x=age, fill=sex)) +
  geom_histogram( color="#e9ecef", alpha=0.6, position = 'identity', binwidth = 2) +
  #scale_fill_manual(values=meta_colors$group) +
  geom_vline(xintercept = 20, linetype="dotted", color = "black", size=1.5) +
  theme_classic() +
...
ggplot(Data_metainfo_New, aes(x=age, y = Data_group, fill=Data_group)) +
  geom_density_ridges() +
  scale_fill_manual(values=meta_colors$group) +
  #geom_vline(xintercept = 20, linetype="dotted", color = "black", size=1.5) +
  theme_ridges() + 
...
```{r}
Age_cat <- as.numeric(Data_metainfo_New$age > 34)
Sex_cat <- Data_metainfo_New$sex

write.csv(Age_cat, file.path(output_dir, "Age_cat.csv"))
write.csv(Sex_cat, file.path(output_dir, "Sex_cat.csv"))

Age_cat <- as.numeric(Data_metainfo_New$age > 20)
...
saveRDS(Data_metainfo_New, file.path(output_dir, "Data_metainfo.RDS"))

write.csv(Data_group, file.path(output_dir, "Data_group.csv"))
write.csv(Data_combining, file.path(output_dir, "Data_combining.csv"))
write.csv(Data_metainfo_New, file.path(output_dir, "Data_metainfo.csv"))
```
