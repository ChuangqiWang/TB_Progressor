...
Temp_index = np.where(Data_Age['x'] == 0)[0]


K_folder = 5
################################################## Create the folder
root = "PerFeatures_ROC_v3_AdolescentNon"
...
for feature_index in range(78, Data_Sero_RiskScore6.shape[1]):
    ...
    #plt.savefig(os.path.join(directory, sel_features_saved + '.Sero.Data.LassoFeatures.HyperParameters.png'))
    plt.savefig(os.path.join(directory, sel_features_saved + '.Sero.Data.LassoFeatures.HyperParameters_MoreSample_STD_V3.pdf'), format="pdf", bbox_inches="tight")
