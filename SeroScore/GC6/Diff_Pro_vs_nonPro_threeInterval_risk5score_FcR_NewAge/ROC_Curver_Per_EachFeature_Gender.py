...
Data_Age = pandas.read_csv('Progressor_Allmonths_NewAge/Sex_cat.csv', index_col = 0)
Temp_index = np.where(Data_Age['x'] == "Female")[0]


K_folder = 5
################################################## Create the folder
root = "PerFeatures_ROC_v3_Female"
...
for feature_index in range(78, Data_Sero_RiskScore6.shape[1]):
    ...
    for iteration in range(50):  
        ...
        ###### 4. ROC plot
        viz = plot_roc_curve_woPred(yhat, y_train, name='Iteration {}'.format(iteration), alpha=0.3, lw=1, ax=ax)
