...
## Load the data
Data_Sero_RiskScore6 = pandas.read_csv('Progressor_Allmonths_NewAge/Data_combining.csv',index_col=0)
Data_Group = pandas.read_csv('Progressor_Allmonths_NewAge/Data_group.csv',index_col=0)


K_folder = 5
################################################## Create the folder
directory = "PerFeatures_ROC_v3"
...
for feature_index in range(78, Data_Sero_RiskScore6.shape[1]):
    sel_features = colnames[feature_index]
    sel_features_saved = sel_features.replace('/', '_')
    ################################################## Create the folder
    directory = "PerFeatures_ROC_v3" + "/" + sel_features_saved
    if not os.path.exists(directory):
        os.makedirs(directory)
    #Run 100 iterations
    ...
    for iteration in range(50):  
        ...
        temp_index = np.argwhere(~np.isnan(X_train))
        X_train = X_train[temp_index, ]
        y_train = y_train[temp_index]
        #Ignore Nan value
        #Test
        X_test = X[test_index, ]
        y_test = y[test_index]
        temp_index = np.argwhere(~np.isnan(X_test))
        X_test = X_test[temp_index, ]
        y_test = y_test[temp_index]
        ...
        ###### 4. ROC plot
        viz = plot_roc_curve_woPred(yhat, y_train, name='Iteration {}'.format(iteration), alpha=0.3, lw=0.4, ax=ax)
        #viz = plot_roc_curve_woPred(X_test, y_test, name='Iteration {}'.format(iteration), alpha=0.3, lw=0.4, ax=ax)
    ...
    #         lw=2, alpha=.8)
    np.savez(os.path.join(directory, 'AUCparameter_mean' + '.npz'), fpr = mean_fpr, auc = mean_auc, tpr = mean_tpr)
    #std AUC
    ...
    ax.legend([handles[index] for index in range(51, 52)], [labels[index] for index in range(51, 52)], loc="lower right")
    # Put a legend to the right of the current axis
    #ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    #plt.savefig(os.path.join(directory, sel_features_saved + '.Sero.Data.LassoFeatures.HyperParameters.png'))
    #plt.savefig(os.path.join("PerFeatures_ROC_v3", sel_features_saved + '.Sero.Data.LassoFeatures.HyperParameters.png'))
    plt.savefig(os.path.join(directory, sel_features_saved + '.Sero.Data.LassoFeatures.HyperParameters_MoreSampleV3.pdf'), format="pdf", bbox_inches="tight")
