...
## Load the data
Data_Sero_RiskScore6 = pandas.read_csv('Progressor_9_18months/Data_combining.csv',index_col=0)
Data_Group = pandas.read_csv('Progressor_9_18months/Data_group.csv',index_col=0)
...
for feature_index in range(78, Data_Sero_RiskScore6.shape[1]):
    ...
    for iteration in range(50):  
        ...
        #
        #Data_Sero = Data_Sero[~np.isnan(Data_Sero)]
        #Dataframe to numpy
        ...
        yhat = X_train
        dataindex = np.argwhere(~np.isnan(yhat))
       
        np.savez(os.path.join(directory, 'Iteration_' + str(iteration) + '_pred_actual_test.npz'), X_test, yhat, y_test)
        
        
        ###### 4. ROC plot
        viz = plot_roc_curve_woPred(yhat[dataindex], y_train[dataindex], name='Iteration {}'.format(iteration), alpha=0.3, lw=1, ax=ax)
