...
## Load the data
Data_Sero_RiskScore6 = pandas.read_csv('SeroScore_substract_zscore/SeroScore.csv',index_col=0)
#Data_Group = pandas.read_csv('Progressor_6months/Data_group.csv',index_col=0)

################################################## Create the folder
directory = "SeroScore_ROC_Substract_zscore"
...
for feature_index in range(Data_Sero_RiskScore6.shape[1]):
    ...
    ################################################## Create the folder
    directory = "SeroScore_ROC_Substract_zscore" + "/" + sel_features_saved
...
df_AUC.to_csv('mean_AUC_SeroScore_zscore.csv', index=False)
